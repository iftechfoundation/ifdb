...
rm -rf initdb
mkdir initdb
echo "create database if not exists ifdb CHARACTER SET latin1 COLLATE latin1_german2_ci; CREATE USER IF NOT EXISTS ifdb; use ifdb;" | cat - $IFDB_FULL_EXPORT/ifdb-*.sql sql/patch-full-schema.sql > initdb/ifdb.sql
...
#cp sql/patch-schema-2.sql initdb/ifdb_patch_schema.sql
